...
<html class="no-js" lang="zxx">
	...
	<body>
		...
		<div class="wrapper">
			...
			<header>
				...
				@include('nguoidung.menu_top')
				<!-- Header Top End Here -->
				<!-- Header Middle Start Here -->
				@include('nguoidung.menu_mid')
										...
										<li>
											...
											<ul class="ht-dropdown megamenu first-megamenu">
												...
												<li class="megamenu-img">
													<a href="shop.html"
														><img
															src="img\vertical-menu\sub-img1.jpg"
															alt="menu-image"
													/></a>
													<a href="shop.html"
														><img
															src="img\vertical-menu\sub-img2.jpg"
															alt="menu-image"
													/></a>
													<a href="shop.html"
														><img
															src="img\vertical-menu\sub-img3.jpg"
															alt="menu-image"
													/></a>
												</li>
				...
				<div class="container">
					<div class="breadcrumb">
						<ul class="d-flex align-items-center">
							...
							<li class="active"><a href="">Danh mục {{ $tenloai }}</a></li>
						</ul>
					</div>
				</div>
				<!-- Container End -->
			...
			<div class="container mt-3">
                ...
                <h5>Có {{ count($list_sp_dm) }} sản phẩm trong danh mục {{ $tenloai }}</h5>
                <div class="row mt-2">
                            ...
                            <div class="pro-content">
                                ...
                                <div class="pro-actions">
                                    <div class="actions-primary">
                                        <a onclick="Addcart({{$sp->ma_sp}})" href="javascript:" title="Add to Cart">
                                            + Thêm vào giỏ</a
                                        >
                                    </div>
                                </div>
                    ...
                    @endforeach
                                    ...
                                    @php
										$phantrang=$list_sp_dm->links()->elements;
										$url=url()->full();
									@endphp
									...
									<div class="pro-pagination">
										<ul class="blog-pagination">
											...
											@endphp
											@foreach($phantrang[0] as $pt)
											@if ($loop->first)
												<li 
												@if($url=='http://127.0.0.1:8000/san-pham-danh-muc/'.$id or $pt==$url)
												class="active"
												@endif
												>
												<a href="{{ $pt }}">{{ ($loop->index)+1 }} </a></li>
    ...
    {{-- <script>
		...
		slide: function (event, ui) {
            ...
            $('#amount').val(
                '' +
                    $('#slider-range').slider('values', 0)+'.VND' +
                    ' - ' +
			...
			function locgia(){
				...
				$.ajax({
            	url:'loc-gia-san-pham/'+$min+'/'+$max+'/'+$("#tt").val()+'/'+$("#ma_loai").val(),
            	type:'GET',

				}).done(function(response){
					$("#loc_sp").empty();
            		$("#loc_sp").html(response);
					

				});
    		}

			function sp_danhmuc(id){
				$min=$('#slider-range').slider('values', 0);
				$max=$('#slider-range').slider('values', 1)
				$.ajax({
            	url:'/san-pham-danh-muc/'+id+'/'+$min+'/'+$max,
            	type:'GET',
